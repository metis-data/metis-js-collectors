...
import { instrument, InstrumentationResult, PlanType, MetisHttpInstrumentation } from '@metis-data/base-interceptor';
...
async function bootstrap() {
  ...
  const sequelizeInstrumentation = getSequelizeInstrumentation(sequelize, PlanType.ESTIMATED, errorHandler);

  const httpInstrumentation = new MetisHttpInstrumentation([/favicon.ico/]);
