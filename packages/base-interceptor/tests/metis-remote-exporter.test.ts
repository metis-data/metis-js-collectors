...
describe('export', () => {
  ...
  const setup = (options?: MetisRemoteExporterOptions) =>
    new MetisRemoteExporter(URL, METIS_API_KEY, { retry: 3, ...(options || {}) });
  ...
  const callExport: (spans: ReadableSpan[]) => Promise<ExportResult> = (spans: ReadableSpan[]) => {
  ...
  it('should handle exception in hook', async () => {
    ...
    await expect(callExport(VALID_SPANS)).resolves.toStrictEqual(SUCCESS_EXPORT);
  });
  ...
  it('should filter spans ', async () => {
    ...
    await expect(callExport([SPAN_RANDOM])).resolves.toStrictEqual(SUCCESS_EXPORT);
